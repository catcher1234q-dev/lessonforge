...
export function SearchEmptyState({
  ...
  return (
    <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-6 py-10 text-center shadow-[0_18px_50px_rgba(15,23,42,0.05)]">
      ...
      <h2 className="mt-4 text-3xl font-semibold text-ink">No demo listings matched that view.</h2>
      <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
        {query
          ? `Try a broader search than "${query}" or switch back to all demo listings.`
          : subject !== "All"
            ? `There are no ${subject} demo listings in this preview right now.`
            : "Clear filters and browse the full demo preview instead."}
      </p>
